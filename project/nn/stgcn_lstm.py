...
class Stgcn_Lstm():
    ...
    def sgcn(self, Input):
        x = keras.layers.Conv2D(filters=64, kernel_size=(1,1), strides=1, activation='relu')(Input)
        x = Dropout(0.25)(x)
        gcn_1 = keras.layers.Lambda(lambda x: tf.einsum('vw,ntwc->ntvc', x[0], x[1]))([self.AD, x])
        y = keras.layers.Conv2D(filters=64, kernel_size=(1,1), strides=1, activation='relu')(Input)
        y = Dropout(0.25)(y)
        gcn_2 = keras.layers.Lambda(lambda x: tf.einsum('vw,ntwc->ntvc', x[0], x[1]))([self.AD2, y])
